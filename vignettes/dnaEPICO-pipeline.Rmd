...
The exported rules include the final report target:

```bash
make reports/model1/docs/index.html MODEL=model1
```

The grouped targets above also refresh the report automatically. For example,
`make f3 MODEL=model1` builds a report with Data, ENmix QC, Quality Control,
Batch Effect, Metrics, Report, and Logs tabs. `make f4`, `make f3lme`, and
`make all` additionally make the GLM and LMER tables available when the
...
The generated site is opened from:
...
the project paths defined by the Makefile variables.
