...
`r Biocpkg("dnaEPICO")` implements methods in `r Biocpkg("minfi")`, `r Biocpkg("ENmix")` and `r Biocpkg("wateRmelon")` packages so there is quite the number of dependencies which need to be installed. This can be handled by simply running:
...
`Note: PDF report generation requires a working LaTeX installation.TinyTeX can be installed in R using: install.packages("tinytex") and tinytex::install_tinytex()`. For troubleshooting LaTeX-related issues, see the [TinyTeX](https://yihui.org/tinytex/) documentation.
...
For this example, the pipeline is run using **one explicit model** (`model1`) rather than multiple parallel models.
...
* `FlowSorted.Blood.450k` is selected as the reference panel because `minfiData` represents blood samples
