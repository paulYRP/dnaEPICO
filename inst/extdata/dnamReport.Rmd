...
```{r, echo=F, include=T, out.width="100%", dpi=300}
# Folder with TIFFs
...
if (dir.exists(dir)) {
  ...
  if (length(files) > 0) {
    for (f in files) {
      ...
      graphics::title(main = basename(f))
    }
  }
}
...
```{r, echo=F, include=T, out.width="50%", dpi=300}
  ...
  for (sub in subdirs) {
    ...
    if (length(sub_files) > 0) {
      ...
      for (f in sub_files) {
        img <- tiff::readTIFF(f)
        graphics::plot(grDevices::as.raster(img))
        graphics::title(main = paste(sub_name, basename(f), sep = " - "))
      }
    }
  }
}
