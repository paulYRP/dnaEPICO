...
dnamReport <- function(
        ...
        if (any(!items$browser_ready)) {
            lines <- c(lines, callout_lines(
                paste(
                    ...
                    "Install `magick` and rerun `dnamReport()` to render these figures in the browser."
                ),
                type = "warning"
            ))
        }
    ...
    make_logs_notes <- function(log_summary, lme_label = "LME Analysis") {
        descriptions <- c(
            methylation = "contains IDAT loading, normalisation, filtering, and cell-composition estimation",
            data = "contains phenotype preparation, timepoint splitting, and methylation-matrix export",
            batch = "contains surrogate-variable analysis for batch-effect assessment",
            glm = "contains GLM fitting and CpG annotation",
            lme = "contains mixed-effects model fitting and CpG annotation"
        )
        labels <- c(
            methylation = "Methylation Analysis", data = "Data Preparation",
        ...
        notes <- c(
            ...
            vapply(log_summary$rows$key, function(key) {
                exists <- isTRUE(log_summary$rows$exists[
                    log_summary$rows$key == key
                ])
                if (!exists) {
                    return(sprintf("`%s` was not found.", labels[[key]]))
                }
                sprintf(
                    "`%s` is available and %s.", labels[[key]],
                    descriptions[[key]]
                )
            }, character(1))
        )
    ...
    build_report_page <- function(project_name, project_dir,
        ...
        overview <-
            "This report summarizes the available datasets, figures, tables, and workflow logs."
    ...
    enmix_notes <- c(paste(
        "`ENmix` produces control plots comparable to Illumina GenomeStudio plots.",
    ...
    metrics_figure_titles <- c(
        "Figure 1: Post-filtering Beta- and M-value Distributions by Timepoint",
        "Figure 2: Post-filtering Principal Component Analysis by Timepoint"
    )
    ...
    quality_control_metadata <- data.frame(
        pattern = c(
            ...
            "methylation beta-value distributions across samples for identifying global patterns and sample outliers",
            "detection p-value distributions across samples for assessing probe-signal reliability and poor detection",
            "sample-level methylated and unmethylated signal intensities for identifying low-quality samples or technical outliers",
            "reported sex distribution for checking sample annotations",
            "sex-related methylation patterns before and after normalisation",
            "methylation-based sex predictions for comparison with reported sex and identification of possible sample or annotation errors"
        ),
        stringsAsFactors = FALSE
    )
    ...
    batch_effect_figure_titles <- c(
        "Figure 1: First Two Surrogate Variables by Sentrix ID",
        "Figure 2: First Three Surrogate Variables by Sentrix ID and Position",
        "Figure 3: First Two Surrogate Variables by Sentrix Position"
    )
    ...
    lme_table_title <- if (has_lme_interaction) {
        sprintf(
            "Table 1. Linear Mixed-Effects Model Results and CpG Annotation by Phenotype and %s Interaction",
    ...
    lme_table_description <- if (isTRUE(lme_table_assets$ok)) {
        ...
        interaction_description <- if (has_lme_interaction) {
            ...
            "No interaction was fitted; the reported p-value columns correspond to phenotype coefficients."
        }
    ...
    base_log_cards <- c(
        ...
        "IDAT loading, normalisation, filtering, and cell-composition estimation.",
        "", "```{r}", "#| echo: false", "#| results: asis",
        sprintf(
            "render_log_block(%s, 'Methylation Analysis')",
            r_string(log_assets$methylation$asset_path)
        ), "```", ":::", "", "::: {.card title=\"Data Preparation\"}",
        "", "Phenotype preparation, timepoint splitting, and methylation-matrix export.",
        "", "```{r}", "#| echo: false", "#| results: asis",
        sprintf(
    ...
    glm_log_card <- if (include_glm) {
        c(
            ...
            "Generalised linear model fitting, association testing, and CpG annotation.",
            "", "```{r}", "#| echo: false", "#| results: asis",
    ...
    lme_log_card <- if (include_lme) {
        c(
            ...
            if (has_lme_interaction) {
                sprintf(
                    "Linear mixed-effects model fitting, the phenotype interaction with %s, and CpG annotation.",
                    lme_interaction_term
                )
            } else {
                "Linear mixed-effects model fitting, phenotype coefficient testing, and CpG annotation."
            }, "", "```{r}", "#| echo: false", "#| results: asis",
            sprintf(
    ...
    if (!magick_available && any(grepl("\\.tiff?$", c(
        ...
        emitLogMinfiEwasWater(
            c(
                ...
                "Install `magick` and rerun this function to generate PNG browser previews."
            ),
            verbose = FALSE, log_path = log_path
        )
    }
